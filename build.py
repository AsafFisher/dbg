...
def run_build(arch):
    ...
    with chdir("./exes/shellcode_linux"):
        os.environ[
            "RUSTFLAGS"
        ] += "-C relocation-model=pie -C link-arg=-nostdlib -C link-arg=-static -C link-arg=-T./shellcode.ld"  # Used to be pie -C target-feature=+crt-static -L/usr/lib/x86_64-linux-musl
        os.system(
            f"cargo +nightly build --bin shellcode --release --verbose -Zbuild-std=core,alloc --target {rust_target}"
        )  #
